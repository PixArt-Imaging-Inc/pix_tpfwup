...
const int Pjp343FlashCtrlr::SRAM_BURST_SIZE = 0x00000100;
const int Pjp343FlashCtrlr::PAGE_SIZE = 256;
const int Pjp343FlashCtrlr::PAGE_COUNT_PERSECTOR = 16;
const int Pjp343FlashCtrlr::SECTOR_COUNT = 15;
const int Pjp343FlashCtrlr::PAGE_COUNT = SECTOR_COUNT * PAGE_COUNT_PERSECTOR;
const int Pjp343FlashCtrlr::SECTOR_SIZE = PAGE_COUNT_PERSECTOR * PAGE_SIZE;

const int Pjp343FlashCtrlr::FIRMWARE_START_PAGE = 0;
const int Pjp343FlashCtrlr::PARAMETER_START_PAGE = 224;

const byte Pjp343FlashCtrlr::IO_BANK = 6;

/* HW SFC register address */

const byte Pjp343FlashCtrlr::REG_SRAM_ACCESS_DATA = 0x0b;

Pjp343FlashCtrlr::Pjp343FlashCtrlr(RegisterAccessor *regAccessor) : FlashController(regAccessor)
{
}

Pjp343FlashCtrlr::~Pjp343FlashCtrlr()
{
}
bool Pjp343FlashCtrlr::flashframeStart()
{
    int retry = 0;
    int value = 0;
    // frame start
...
bool Pjp343FlashCtrlr::writeEnable()
{
    int retry = 0;
    int value = 0;
...
bool Pjp343FlashCtrlr::checkBusy()
{
    int retry = 0;
    int value;
    while (true)
...
void Pjp343FlashCtrlr::powerOnFlashcontroller()
{
    mRegAccessor->writeRegister(1, 0x0d, 0x02);
}
void Pjp343FlashCtrlr::enterEngineerMode()
{
...
bool Pjp343FlashCtrlr::exitEngineerMode()
{
    int i = 0;
    byte res;
    mRegAccessor->writeRegister(1, 0x2c, 0xaa);
...
int Pjp343FlashCtrlr::writeFlash(byte *data, int length, int startPage,
                                 bool doErase)
{
#ifdef DEBUG
...
int Pjp343FlashCtrlr::readFlash(byte **data, int startPage, int pageLength)
{
    ...
    *data = pData;
    ...
    for (int i = 0; i < dataSize; i += SRAM_BURST_SIZE)
    {
        ...
        int readSize =
            remainedSize < SRAM_BURST_SIZE ? remainedSize : SRAM_BURST_SIZE;
        int startReadSector = (startPage + i / PAGE_SIZE) / PAGE_COUNT_PERSECTOR;
...
bool Pjp343FlashCtrlr::erase(int startPage, int pageLength)
{
    if (startPage % PAGE_COUNT_PERSECTOR != 0)
        return false;
...
void Pjp343FlashCtrlr::setFlashAddress(unsigned int addr)
{
...
bool Pjp343FlashCtrlr::eraseWithSector(byte sector, byte length)
{
#ifdef DEBUG
...
bool Pjp343FlashCtrlr::writeSram(byte *data, int length)
{
#ifdef DEBUG
    ...
    mRegAccessor->writeRegister(IO_BANK, 0x09, 0x0f); // sram select QSPI Buffer
...
bool Pjp343FlashCtrlr::writeSramToFlash(uint32_t page, byte length)
{
#ifdef DEBUG
    printf("writeSramToFlash(), page=%d, length=%d\n", page, length);
#endif // DEBUG
    bool res = true;
    do
    {
...
void Pjp343FlashCtrlr::readFlashToSram(byte sector, byte length)
{
}

int Pjp343FlashCtrlr::readSram(byte *data, int length)
{
#ifdef DEBUG
...
void Pjp343FlashCtrlr::readSysRegisterBatch(byte bank, int length, bool AutoRead)
{
...
void Pjp343FlashCtrlr::readUserRegisterBatch(byte bank, int length, bool AutoRead)
{
    byte *pData = new byte[length];
...
void Pjp343FlashCtrlr::writeRegister(byte bank, byte addr, byte value)
{
    mRegAccessor->writeRegister(bank, addr, value);
}
void Pjp343FlashCtrlr::writeUserRegister(byte bank, byte addr, byte value)
{
    mRegAccessor->writeUserRegister(bank, addr, value);
}
void Pjp343FlashCtrlr::readFrame(void)
{
