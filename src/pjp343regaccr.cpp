...
Pjp343RegAccr::Pjp343RegAccr(HidDevice *dev) : mHidDevice(dev), mMaxBurstLength(0)
{
}

Pjp343RegAccr::~Pjp343RegAccr()
{
}

void Pjp343RegAccr::prepare()
{
    /**
...
byte Pjp343RegAccr::readuserRegister(byte bank, byte address)
{
    byte buffer[4];
    int len = 0;
...
byte Pjp343RegAccr::readRegister(byte bank, byte address)
{
    byte buffer[4];
    int len = 0;
...
int Pjp343RegAccr::readRegisters(byte *data, byte bank, byte startAddress,
                                 uint64_t length)
{
...
int Pjp343RegAccr::readUserRegisters(byte *data, byte bank, byte startAddress,
                                     uint64_t length)
{
    for (uint64_t i = 0; i < length; ++i)
    {
...
int Pjp343RegAccr::burstReadRegister(byte *data, byte bank, byte address,
                                     uint64_t length)
{
...
void Pjp343RegAccr::writeRegister(byte bank, byte address, byte value)
{
    byte buffer[] =
        {address, bank, value};
    mHidDevice->setFeature(RW_REG_REPORT_ID, buffer, 3);
}
void Pjp343RegAccr::writeUserRegister(byte bank, byte address, byte value)
{
    byte buffer[] =
        {address, bank, value};
    mHidDevice->setFeature(RW_USER_REG_REPORT_ID, buffer, 3);
}
void Pjp343RegAccr::writeRegisters(byte bank, byte startAddress, byte *values,
                                   uint64_t length)
{
    for (uint64_t i = 0; i < length; ++i)
    {
...
byte Pjp343RegAccr::readInputReport(void)
{
    bool res = mHidDevice->Read();
    return res;
}
void Pjp343RegAccr::burstWriteRegister(byte bank, byte address, byte *values,
                                       uint64_t length)
{
#ifdef DEBUG
